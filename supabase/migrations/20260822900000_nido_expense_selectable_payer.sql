-- Selectable expense payer (household member, not only the writer)
--
-- Product: Registrar un gasto lets the writer pick who paid. Default is the
-- current user. created_by stays auth.uid(). payer_id may be any active member
-- of the same household. Other-household and fake UUIDs stay denied.
--
-- RLS INSERT no longer requires payer_id = auth.uid(). RLS UPDATE no longer
-- requires payer_id = auth.uid(), so a creator can edit an expense someone else
-- paid and can correct the payer. Integrity still requires a newly assigned
-- payer to be an active member.
--
-- create_expense signature is unchanged. update_expense gains p_payer_id.
-- SECURITY INVOKER RPCs stay the write path.
...
COMMENT ON FUNCTION public.create_expense(uuid, uuid, numeric, text, date, uuid, public.expense_scope, jsonb) IS
  'Inserts a confirmed expense and its splits in one transaction. SECURITY INVOKER. created_by is auth.uid(). payer_id may be any active household member. Personal ignores households.default_split_method (fixed). Shared equal keeps client equal splits. Shared proportional recomputes from confirmed incomes of the current America/Mexico_City month and stores income_based.';

DROP FUNCTION IF EXISTS public.update_expense(uuid, uuid, numeric, text, date, public.expense_scope, jsonb);
...
BEGIN
  ...
  IF p_payer_id IS DISTINCT FROM v_payer_id
...
COMMENT ON FUNCTION public.update_expense(uuid, uuid, numeric, text, date, uuid, public.expense_scope, jsonb) IS
  'Updates a confirmed expense, its payer, and replaces its splits in one transaction. SECURITY INVOKER. Only the creator with an active membership may mutate a non-deleted expense. payer_id may change to another active household member.';

REVOKE ALL ON FUNCTION public.update_expense(uuid, uuid, numeric, text, date, uuid, public.expense_scope, jsonb) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.update_expense(uuid, uuid, numeric, text, date, uuid, public.expense_scope, jsonb) TO authenticated;
