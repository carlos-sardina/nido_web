...
import { canSubmitRecurrence, materializeRecurringExpense, setRecurringExpenseActive } from "@/lib/nido/recurring-expenses";
import { P } from "@/lib/palette";

export function RecurringExpenseDetail({
  template,
  currentUserId,
  onClose,
  onEdit,
            ...
            <ScreenFooter>
              <div className="space-y-2">
                {due ? (
                  <Button
                    loading={submitting}
                    onClick={() => void run(() => materializeRecurringExpense(template.id, template.nextOccurrence))}
                  >
        ...
        <div className="space-y-3">
            ...
            <div className="rounded-2xl p-4" style={{ backgroundColor: P.card }}>
              ...
              <Text size="caption" tone="muted" className="mt-3">
                ...
                {template.scope === "personal" ? "Personal" : "Compartido"} · {recurrenceStatusLabel(status)}
              </Text>
              ...
              {due ? (
                ...
                <Text size="caption" className="mt-2" tone="muted">
                  El próximo periodo se podrá registrar a partir de esa fecha. Guardar la plantilla no crea gastos futuros.
                </Text>
              )}
            </div>
